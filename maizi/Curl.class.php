...
class Curl
{
    ...
    function status($debug = 0)
    {
        if ($debug == 2) {
            ...
            $data = [
                'finish' => $this->finishNum,
                'cache' => $this->cacheNum,
                'task' => $this->taskNum,
                'active' => $this->activeNum,
                'running' => count($this->taskRunning),
                'queue' => $this->queueNum,
                'failed' => $this->failedNum,
                'taskPool' => count($this->taskPool),
                'msg' => $s,
            ];
    ...
    function read($url)
    {
        ...
        if (curl_errno($ch) === 0) {
            ...
            var_dump('error: code ' . curl_errno($ch) . ", " . curl_error($ch), E_USER_WARNING);
        }
        return $r;
    }
    ...
    function add($url = array(), $p = array(), $f = array())
    {
        ...
        if (!is_array($url) or empty($url[0])) {
            var_dump($url);
            var_dump('url is invalid', E_USER_ERROR);
        }
        if (!is_array($p) or !is_array($f))
            var_dump('callback is not array', E_USER_ERROR);
        if (!isset($p[0]))
            var_dump('process callback is not set', E_USER_ERROR);
        if ((isset($p[1]) and !is_array($p[1])) or (isset($f[1]) and !is_array($f[1]))) {
            var_dump('callback function parameter must be an array', E_USER_ERROR);
        }
        //fix
        if (empty($url[1]))
            $url[1] = null;
    ...
    function go()
    {
        ...
        if ($running)
            var_dump('CURL can only run one instance', E_USER_ERROR);
        ...
        do {
            ...
            while ($curlInfo = curl_multi_info_read($this->mh, $this->queueNum)) {
                ...
                if (empty($task)) {
                    var_dump("can't get running task", E_USER_WARNING);
                }
                $callFail = false;
            ...
            if (!empty($task)) {
                ...
                if (!$cache) {
                    ...
                    if (is_resource($ch)) {
                        ...
                        var_dump('$ch is not resource,curl_init failed.', E_USER_WARNING);
                    }
                }
            }
            if (!$cache)
    ...
    private function cache($url, $content = null)
    {
        ...
        if (!isset($this->cache['dir']))
            var_dump('Cache dir is not defined', E_USER_ERROR);
        ...
        if (!isset($content)) {
            ...
            if (!is_dir($this->cache['dir'])) {
                var_dump("Cache dir doesn't exists", E_USER_ERROR);
            } else {
                $dir = dirname($file);
                if (!file_exists($dir) and !mkdir($dir, 0777))
                    var_dump("Create dir failed", E_USER_WARNING);
                $content = serialize($content);
                if (file_put_contents($file, $content, LOCK_EX))
                    $r = true;
                else
                    var_dump('Write cache file failed', E_USER_WARNING);
            }
            return $r;
        }
    }
